// assets/js/admin/event/ponentes_evento/editar_asignacion/validacion.js
...
// ---- Validación para el submit (usada por form.js) ----
export function validateForm() {
  const required = [
    {
      name: "speaker_event[hora_participacion]",
      label: "Hora de Participación",
    },
...
function setupTimeValidation() {
  ...
  const handler = (e) => validateParticipationTime(e.target.value);
  horaField.addEventListener("input", handler);
  horaField.addEventListener("blur", handler);
}
...
function validateParticipationTime(timeValue) {
  ...
  const eventStart = String(data.hora_inicio).substring(0, 5);
  const eventEnd = String(data.hora_fin).substring(0, 5);
...
function showTimeWarning(eventStart, eventEnd, selectedTime) {
  ...
  warning.className =
    "time-validation mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg";
  warning.innerHTML = `
      <div class="flex items-start gap-2">
        ...
        <div class="text-sm">
          ...
          <p class="text-yellow-700 mt-1">
            El evento es de <strong>${formatTime(
              eventStart
            )}</strong> a <strong>${formatTime(eventEnd)}</strong><br>
...
function showTimeSuccess() {
  ...
  success.className =
    "time-validation mt-2 p-2 bg-green-50 border border-green-200 rounded-lg";
  success.innerHTML = `
...
function clearTimeWarnings() {
  document.querySelectorAll(".time-validation").forEach((el) => el.remove());
}
...
function setupStateValidation() {
  ...
  const h = () => validateStateAndCertificate();
  estadoField.addEventListener("change", h);
  certificadoField.addEventListener("change", h);
}
...
function validateStateAndCertificate() {
  ...
  const estado = String(estadoField.value || "").toLowerCase();
...
function showStateMessage(title, message, type) {
  const estadoField = document.getElementById("estado_asistencia");
  ...
  node.className = `state-validation mt-4 p-3 ${cfg.bg} ${cfg.border} border rounded-lg`;
  node.innerHTML = `
      <div class="flex items-start gap-2">
        <svg class="w-4 h-4 ${cfg.icon} mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="${cfg.iconPath}" clip-rule="evenodd"/>
        </svg>
        <div class="text-sm">
          <p class="${cfg.text} font-medium">${title}</p>
          <p class="${cfg.text} mt-1">${message}</p>
        </div>
      </div>
    `;
...
function clearStateWarnings() {
  document.querySelectorAll(".state-validation").forEach((el) => el.remove());
}
