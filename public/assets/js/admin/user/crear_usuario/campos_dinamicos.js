import {
  ...
  establecerRequeridosCamposPonente,
  establecerRequeridosCamposInvitado,
} from "./helpers.js";
...
export function configurarEventListeners() {
  ...
  if (programaSelect)
    programaSelect.addEventListener("change", manejarCambioProgramaAcademico);
}
...
function manejarCambioTipoInvitado() {
  ...
  const camposProgramaAcademico = document.getElementById(
    "camposProgramaAcademico"
  );
  ...
  switch (tipoValue) {
    case "estudiante":
      if (camposProgramaAcademico)
        camposProgramaAcademico.classList.remove("hidden");
