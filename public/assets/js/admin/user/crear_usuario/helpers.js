...
export function limpiarCamposEspecificos() {
    const ponenteFields = ["tema", "descripcionBiografica", "especializacion", "institucionPonente"];
    const invitadoFields = ["tipoInvitado", "correoInstitucional", "programaAcademico", "nombreCarrera", "jornada", "facultad", "cargo", "sedeInstitucion"];
...
export function establecerRequeridosCamposPonente(requerido) {
    ["tema", "descripcionBiografica", "especializacion", "institucionPonente"].forEach(campo => {
        const el = document.getElementById(campo);
