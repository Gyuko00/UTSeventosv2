    ...
    usuarios.forEach(usuario => {
        ...
        html += `
            <tr class="hover:bg-gray-50 usuario-row"
                data-nombre="${(usuario.nombres + ' ' + usuario.apellidos).toLowerCase()}"
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <div class="text-sm text-gray-500">${escapeHtml(usuario.correo_personal || 'Sin email')}</div>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">${escapeHtml(usuario.nombres + ' ' + usuario.apellidos)}</div>
                    <div class="text-sm text-gray-500">${escapeHtml(usuario.tipo_documento + ': ' + usuario.numero_documento)}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <a href="${window.location.origin}/utseventos/public/admin/detalleUsuario/${usuario.id_usuario}" class="text-blue-600 hover:text-blue-900 mr-3" title="Ver detalle"><i class="fas fa-eye"></i></a>
                    <a href="${window.location.origin}/utseventos/public/admin/editarUsuario/${usuario.id_usuario}" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Editar"><i class="fas fa-edit"></i></a>
                    <button onclick="confirmarEliminacion(${usuario.id_usuario}, '${escapeHtml(usuario.usuario)}')" class="text-red-600 hover:text-red-900" title="Eliminar"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`;
    });
