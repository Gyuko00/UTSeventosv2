<?php // app/modules/admin/components/eventos/crud/editar/ubicacion_fecha.php ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        <select
          ...
          name="event[departamento_evento]"
          class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-lime-500 focus:border-transparent transition-all duration-200"
          data-selected="<?= htmlspecialchars($evento['departamento_evento'] ?? '') ?>"
        >
      ...
      <div>
        ...
        <select
          id="municipio_evento"
          name="event[municipio_evento]"
          class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-lime-500 focus:border-transparent transition-all duration-200"
          data-selected="<?= htmlspecialchars($evento['municipio_evento'] ?? '') ?>"
        >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        <input
          ...
          name="event[institucion_evento]"
          maxlength="100"
          class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-lime-500 focus:border-transparent transition-all duration-200"
          placeholder="Nombre de la institución organizadora"
          value="<?= htmlspecialchars($evento['institucion_evento'] ?? '') ?>"
        >
      ...
      <div>
        ...
        <input
          ...
          value="<?= htmlspecialchars($evento['lugar_detallado'] ?? '') ?>"
        >
        <p class="text-sm text-gray-500 mt-1">Máximo 100 caracteres</p>
      </div>
    </div>
...
<?php if (!empty($evento['departamento_evento'])): ?>
  <script>
    window.DEPARTAMENTO_SELECCIONADO = "<?= htmlspecialchars($evento['departamento_evento']) ?>";
    window.MUNICIPIO_SELECCIONADO = "<?= htmlspecialchars($evento['municipio_evento'] ?? '') ?>";
  </script>
<?php endif; ?>
