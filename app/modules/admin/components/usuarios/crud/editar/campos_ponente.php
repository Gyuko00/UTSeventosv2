<div id="camposPonente" class="bg-white p-6 rounded-lg shadow-sm <?= ($usuarioData['id_rol'] ?? null) == 2 ? '' : 'hidden' ?>">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div class="md:col-span-2">
      <label for="tema" class="block text-base font-semibold text-gray-700 mb-2">Tema *</label>
      <input
        type="text"
        name="roleSpecific[tema]"
        id="tema"
        value="<?= htmlspecialchars($usuarioData['tema'] ?? '') ?>"
    ...
    <div class="md:col-span-2">
      <label for="descripcionBiografica" class="block text-base font-semibold text-gray-700 mb-2">Descripción Biográfica *</label>
      <textarea
        name="roleSpecific[descripcion_biografica]"
        id="descripcionBiografica"
        rows="4"
        class="w-full px-4 py-2 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      ><?= htmlspecialchars($usuarioData['descripcion_biografica'] ?? '') ?></textarea>
      <span class="text-red-500 text-xs hidden" id="error-descripcionBiografica"></span>
    </div>

    <div>
      <label for="especializacion" class="block text-base font-semibold text-gray-700 mb-2">Especialización *</label>
      <input
        type="text"
        name="roleSpecific[especializacion]"
        id="especializacion"
        value="<?= htmlspecialchars($usuarioData['especializacion'] ?? '') ?>"
        class="w-full px-4 py-2 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      />
      <span class="text-red-500 text-xs hidden" id="error-especializacion"></span>
    </div>

    <div>
      <label for="institucionPonente" class="block text-base font-semibold text-gray-700 mb-2">Institución del Ponente *</label>
      <input
        type="text"
        name="roleSpecific[institucion_ponente]"
        id="institucionPonente"
        value="<?= htmlspecialchars($usuarioData['institucion_ponente'] ?? '') ?>"
        class="w-full px-4 py-2 text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      />
      <span class="text-red-500 text-xs hidden" id="error-institucionPonente"></span>
    </div>
  </div>
</div>
